### Has a runtime of O(B^d)
### A search algorith that finds the shortest path between two points
### Examples include:
###    1. The Dijkstra's algorithm
###       -- Not best to use when searching for a single path from a single input more useful When Looking for all avalible paths from a single input
###    2. The Breadth-First Search algorithm
###    3. The Depth-First Search algorithm
###    4. The A* algorithm(Greedy Algorith)
###       -- A modified version of Dijkstra’s algorithm. A* works by having both a starting vertex and a target vertex. The algorithm computes an estimated distance, or heuristic, for all possible paths between the starting vertex and the goal vertex and then selects the shortest one.
###       --Created from Dijkstra's algorithm by:
###          1. Adding/Defining a search target
###          2. Gather possible optimal paths and identify a single shortest path
###          3. Adding a heuristic to the search target to determine the likely distance remaining
###Refer to greedy_algorithm.py for more information
